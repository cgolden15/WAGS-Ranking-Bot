...
exports.run = async (client, message, args) => {
    ...
    try {
        ...
        client.users.cache.get("317283391982534666").send('An error occured when running the fire command.\n \n https://replit.com/@cgolden15/WAGS-Ranking-Bot#commands/index.js');
        return message.channel.send({embed: {
            color: 16733013,
    ...
    message.channel.send({embed: {
        color: 9240450,
        description: `**Success!** Fired ${username} to ${fireResponse.name} (${fireResponse.rank})`,
        author: {
            name: message.author.tag,
            icon_url: message.author.displayAvatarURL()
        }
    ...
    logchannel.send({embed: {
        color: 2127726,
        description: `<@${message.author.id}> has fired ${username} from ${rankNameInGroup} (${rankInGroup}) to ${fireResponse.name} (${fireResponse.rank}).`,
        author: {
            name: message.author.tag,
            icon_url: message.author.displayAvatarURL()
        },
